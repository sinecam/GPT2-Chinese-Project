...
The frozen `eval/zh_generation_v1.jsonl` suite contains 37 tasks across:
...
python scripts/benchmark_zh.py \
  ...
  --suite eval/zh_generation_v1.jsonl \
  --out_dir reports/V4.4_vs_V4.5_zh_v1 \
  --device cuda \
  --dtype bfloat16 \
  --temperature 0 \
  --max_new_tokens 160 \
...
python scripts/score_blind_review.py \
  --input reports/V4.4_vs_V4.5_zh_v1/reviewer_1.csv \
          reports/V4.4_vs_V4.5_zh_v1/reviewer_2.csv \
  --key reports/V4.4_vs_V4.5_zh_v1/blind_key.json \
  --out_dir reports/V4.4_vs_V4.5_zh_v1/human
```
...
Never add benchmark prompts or reference answers to SFT data. Do not edit `zh_generation_v1.jsonl`; create `zh_generation_v2.jsonl` when the suite must change.
