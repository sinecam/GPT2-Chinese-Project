...
FINAL_RUNTIME_CONFIG = {
    "system_prompt": "你是一个中文AI助手。请使用简体中文回答用户问题，尽量准确、简洁，并严格遵守用户要求。",
    "max_new_tokens": 120,
    "temperature": 0.0,
    "top_k": 30,
    "top_p": 0.85,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="LABEL=CKPT",
        help="Repeat for every model, for example --model final=checkpoints/final/ckpt.pt",
    )
    parser.add_argument("--tokenizer", required=True)
    parser.add_argument("--suite", default="eval/zh_generation_v3.jsonl")
    parser.add_argument("--out_dir", default="reports/final")
    parser.add_argument(
        "--system_prompt",
        default=FINAL_RUNTIME_CONFIG["system_prompt"],
    )
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--dtype", choices=("float32", "float16", "bfloat16"), default="bfloat16")
    parser.add_argument("--max_new_tokens", type=int, default=FINAL_RUNTIME_CONFIG["max_new_tokens"])
    parser.add_argument("--temperature", type=float, default=FINAL_RUNTIME_CONFIG["temperature"])
    parser.add_argument("--top_k", type=int, default=FINAL_RUNTIME_CONFIG["top_k"])
    parser.add_argument("--top_p", type=float, default=FINAL_RUNTIME_CONFIG["top_p"])
    parser.add_argument("--repetition_penalty", type=float, default=FINAL_RUNTIME_CONFIG["repetition_penalty"])
    parser.add_argument("--no_repeat_ngram_size", type=int, default=FINAL_RUNTIME_CONFIG["no_repeat_ngram_size"])
...
@torch.no_grad()
def encode_prompt(sp, text: str):
    """Encode a prompt string with SentencePiece and return token ids."""
...
def main() -> None:
    ...
    for model_index, (label, checkpoint_path) in enumerate(models):
        ...
        for task_index, task in enumerate(tasks):
            formatted_prompt = engine.format_prompt([], task["prompt"], args.system_prompt, 6)
