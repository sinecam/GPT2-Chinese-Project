...
from scripts.sft_common import (
    fingerprint,
    ...
    for request, answer in refusal_cases:
        for prefix in refusal_prefixes:
            for suffix in refusal_suffixes:
                refusals.append(
                    row("safety_refusal", f"{prefix}{request}{suffix}", answer, "final_curated")
                )
    ...
    for statement, answer in truth_cases:
        for prefix in truth_prefixes:
            for suffix in truth_suffixes:
                corrections.append(
                    row("truth_correction", f"{prefix}{statement}{suffix}", answer, "final_curated")
                )
...
def generate_strict_rows() -> dict[str, list[dict[str, str]]]:
    ...
    for value in fixed_values:
        for template in exact_templates:
            for suffix in exact_suffixes:
                exact_rows.append(
                    row("strict_exact", template.format(value=value) + suffix, value, "final_generated")
                )
    ...
    for source, answer in summaries:
        for prefix in summary_prefixes:
            for suffix in bounded_suffixes:
                bounded_rows.append(
                    row("strict_bounded", f"{prefix}{source}。{suffix}", answer, "final_curated")
                )
    ...
    for topic, answer in two_sentence_cases:
        for template in sentence_templates:
            for suffix in bounded_suffixes:
                bounded_rows.append(
                    row("strict_bounded", template.format(topic=topic) + suffix, answer, "final_curated")
                )
    ...
    for question, answer in facts:
        for prefix in prefixes:
            for suffix in suffixes:
                values.append(
                    row("factual", f"{prefix}{question}{suffix}", answer, "final_curated")
                )
    return values
        ...
        for b in range(2, 50):
            if (a, b) != (17, 28):
                for template in add_templates:
                    groups["add"].append(
                        row("reasoning_add", template.format(a=a, b=b), str(a + b), "final_generated")
                    )
            if a >= b:
                for template in sub_templates:
                    groups["sub"].append(
                        row("reasoning_sub", template.format(a=a, b=b), str(a - b), "final_generated")
                    )

    for a in range(2, 21):
        for b in range(2, 13):
            groups["mul"].append(
                row("reasoning_mul", f"计算{a}乘{b}，只写数字。", str(a * b), "final_generated")
            )
            groups["mul"].append(
                row("reasoning_mul", f"{a}×{b}等于多少？不要写过程。", str(a * b), "final_generated")
            )
            groups["div"].append(
                row("reasoning_div", f"{a * b}除以{b}等于多少？只写数字。", str(a), "final_generated")
            )
            groups["div"].append(
                row("reasoning_div", f"完成整除：{a * b}÷{a}。直接给答案。", str(b), "final_generated")
            )
...
def generate_anti_echo_rows() -> dict[str, list[dict[str, str]]]:
        ...
        for time in times:
            for room in rooms:
                for topic in topics:
                    ...
                    notices.append(row("anti_echo_notice", instruction, answer, "final_generated"))
        ...
        for reason in reasons:
            for day in days:
                for prefix in email_prefixes:
                    ...
                    emails.append(row("anti_echo_email", instruction, answer, "final_generated"))
    ...
    for source, answer in summary_cases:
        for prefix in summary_prefixes:
            summaries.append(row("anti_echo_summary", f"{prefix}{source}。", answer, "final_curated"))
    ...
    for source, answer in rewrite_cases:
        for prefix in rewrite_prefixes:
            rewrites.append(row("anti_echo_rewrite", f"{prefix}{source}。", answer, "final_curated"))

    return {"notice": notices, "email": emails, "summary": summaries, "rewrite": rewrites}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the final response-only Chinese SFT mix.")
    parser.add_argument("--input", nargs="+", required=True, help="Base Chinese SFT JSON/JSONL source.")
    parser.add_argument("--out", default="data/raw/final_sft_mix.jsonl")
    ...
    for record in iter_records(args.input):
        ...
        replay_candidates.append(
            (
                ...
                rng.random(),
                row("broad_replay", normalized["instruction"], normalized["output"], "base_replay"),
            )
        )
