...
def generate_knowledge_candidates() -> list[dict[str, str]]:
    ...
    prefixes = ("请用一句话解释：", "面向初学者说明：", "简洁回答：", "请给出准确解释：", "用不超过两句话说明：", "请不要列举无关内容。问题：", "请直接回答概念：", "用通俗中文解释：")
...
def generate_writing_candidates() -> list[dict[str, str]]:
    ...
    prefixes = ("请完成以下写作任务：", "按照要求直接给出正文：", "请不要复述题目：", "用简洁自然的中文完成：", "请直接写结果：", "请注意格式要求：", "请控制篇幅：", "写作任务：", "请给出可直接使用的文本：", "不要解释写作过程：")
    values = []
